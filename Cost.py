...
def cost_main_fn(aimodel, cost_params, A_cluster, A_mod_dict, clusters,A_d2d_mod,stack_area_dict,stack_ids, n_pins_dict, main_dir):
    ...
    #import pdb; pdb.set_trace()
    chip_ip_dict={}
    for chip_idx, A_chip in enumerate(A_cluster):
        ...
        #import pdb; pdb.set_trace()
        ip_set_chip = set(clusters[chip_idx]) 
        if tuple(sorted(ip_set_chip)) not in chip_ip_dict :
            C_nre = cost_nre_chiplet(A_chip, cost_params["k_chip_nre"], A_mod_dict[chip_idx], cost_params["k_mod_nre"], cost_params["C_chip_nre_fixed"])
            #A_d2d = G_sys.nodes[chip_idx].get("2.5D link Area", 0)
            C_nre += cost_nre_d2d(A_d2d_mod[chip_idx], cost_params["C_D2D_nre"])* 4 #Assuming 4 different D2D connections per chiplet - North, South, East, West
        ...
        #print(chip_ip_dict)
        #import pdb; pdb.set_trace()
    for stack_idx, A_bond in stack_area_dict.items():
        if A_bond != 0:
            N_pins = math.ceil(n_pins_dict["die2interposer"][stack_idx]/0.7) if "die2interposer" in n_pins_dict else 0 #70% signal and 30% power/ground 
            ...
            if N_dies>1:
                ...
                N_pins_3d= math.ceil(n_pins_dict["die2die"][stack_idx]/0.7) if "die2die" in n_pins_dict else 0 #70% signal and 30% power/ground
    ...
    A_sub =1
    A_interposer =0
    if len(stack_area_dict)>1:
        A_interposer = sum(stack_area_dict.values())*cost_params["si_scaling_factor"]
    ...
    A_sub *= sum(stack_area_dict.values())*cost_params["sub_scaling_factor"]
    N_sub = 1
    ...
    NRECost = sum(cost_breakdown["nre"].values())
    RECost = total_cost - NRECost
    #print("----------Cost Summary---------------")
    ...
    if DEBUG:
        ...
        plt.close() 
    #import pdb; pdb.set_trace()
    return total_cost, cost_breakdown,  total_cost/manuf_vol, [NRECost], [RECost]